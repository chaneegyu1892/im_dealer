...
**전이 우선순위: 만료가 지급 자격보다 앞선다.** `HELD` 쿠폰이 유효기간을 넘긴 상태에서
계약이 확인되면 `PENDING` 이 아니라 `EXPIRED` 로 간다. 자격 판정을 먼저 하면, 만료
스윕이 그 전에 돌았는지에 따라 같은 조건의 회원이 받기도 하고 못 받기도 한다 — 지급
여부가 회원의 방문 이력에 좌우되는 셈이라 돈이 나가는 규칙으로 성립하지 않는다.
...
| `src/app/api/admin/quotes/[id]/route.ts` | `status` 가 `CONVERTED` 로 바뀌거나 `CONVERTED` 에서 벗어날 때 |

세 번째가 B 단독의 유일한 실질 결함을 없앤다. `CONVERTED` 전이는 코드 전체에서 이 한
곳뿐이라 훅 비용이 작다. 나중에 크론이 필요해지면 같은 함수를 전수로 돌린다.

**훅은 양방향으로 걸어야 한다.** `CONVERTED` 로 들어올 때만 걸면 계약 철회
(`CONVERTED` → `LOST`)가 아무것도 동기화하지 않아 `PENDING` 쿠폰이 그대로 남고,
어드민 지급 목록에 존재하지 않는 계약의 쿠폰이 계속 보인다. 회원이 마이페이지를 열면
정리되지만, 철회된 회원에게는 방문할 이유가 없다. 그래서 조건은 "상태가 바뀌었고
그 양쪽 중 하나가 `CONVERTED`"다.
