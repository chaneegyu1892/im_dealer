...
하지 않는다. 신규 질문 흐름은 고객 유형 뒤에 공통 월예산을 받으며, 이 예산은
`60개월 / 연 2만km / 무보증(보증금·선납금 0원)` 대표 금액의 상한으로 적용한다.
과거 세션의 `industryDetail`은 결과 근거 호환을 위해 받을 수 있지만 점수에는 사용하지 않는다.
...
최종 선택은 제외 차량·운영 가능성·연료·예산·최신 연식 중복 제거를 먼저 적용한다.
선택한 주용도·추가 조건과 명시적으로 충돌하는 차량은 제외한 뒤, 2026년 5월 신차
신규등록 순위 30위 안의 호환 차량을 미순위 호환 차량보다 먼저 선택한다. 같은 계층
안에서는 `rankScore desc → 신규등록 순위 asc → modelYear desc → companyPriority desc
→ profitPriority desc → slug asc` 순으로 정렬한다. 회사/수익 우선순위 초기값은 0이며
관리자가 설정하기 전에는 동률이다.
