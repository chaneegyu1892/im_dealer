...
export async function GET(request: Request) {
  ...
  const account = useSync ? await fetchKakaoAccount(providerToken!) : null;
  const channelId = process.env.KAKAO_CHANNEL_ID?.trim();
  const channelRelation =
    useSync && channelId ? await getChannelRelation(providerToken!, channelId) : null;
  const agreedTags = useSync ? await fetchAgreedTermTags(providerToken!) : [];

  const marketingTag = process.env.KAKAO_MARKETING_TERMS_TAG?.trim() || "marketing";
  // 채널추가 동의항목이 광고성 정보 수신 동의를 겸하는 구성이라 둘 중 하나면 true.
  const marketingConsent = channelRelation === "ADDED" || agreedTags.includes(marketingTag);
