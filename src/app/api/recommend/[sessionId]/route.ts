...
async function resultIsSqlNull(logId: string, result: unknown): Promise<boolean> {
  if (result !== null) return false;
  const rows = await prisma.$queryRaw<{ readonly isSqlNull: boolean }[]>`
    SELECT "result" IS NULL AS "isSqlNull"
    FROM "RecommendationLog"
    WHERE "id" = ${logId}
  `;
  return rows.length === 1 && rows[0]?.isSqlNull === true;
}
...
export async function GET(
  ...
  try {
    ...
    const isMember = !!(await getActiveUser());
    ...
    const stored = parseStoredResultState(
      log.result,
