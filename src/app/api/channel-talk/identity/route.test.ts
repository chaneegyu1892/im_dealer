...
const mocks = vi.hoisted(() => ({ getActiveUser: vi.fn() }));
...
describe("GET /api/channel-talk/identity", () => {
  ...
  it("카카오싱크로 받은 이름·이메일·전화번호를 프로필에 싣는다", async () => {
    ...
    mocks.getActiveUser.mockResolvedValue(member({ phone: "+82 10-8481-3357" }));
  ...
  it("싱크 이전 가입자는 이름만 실린다(미수집 필드는 키 자체를 뺀다)", async () => {
    mocks.getActiveUser.mockResolvedValue(member({ email: null, phone: null }));
  ...
  it("변환 불가한 전화번호는 프로필에서 제외한다", async () => {
    mocks.getActiveUser.mockResolvedValue(member({ phone: "연락처 없음" }));
  ...
  it("memberHash 는 시크릿 hex 디코딩 키로 만든 SHA256 이다", async () => {
    mocks.getActiveUser.mockResolvedValue(member());
