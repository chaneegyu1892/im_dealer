...
import { vehicleImageVisibilitySchema } from "@/lib/validations/admin-vehicle-images";
import { setVehicleImageVisibility } from "@/lib/vehicle-images/item-mutations";
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const parsed = vehicleImageVisibilitySchema.safeParse(body.value);
    if (!parsed.success) return invalidInput(parsed.error.flatten());
    const result = await setVehicleImageVisibility(route.id, route.imageId, parsed.data);
    const before = imageResponse(result.before);
    const image = imageResponse(result.image);
    await logAdminAction({ request, actor: admin, action: "VEHICLE_IMAGE_VISIBILITY", resource: "VehicleImage", targetId: route.imageId, before, after: image });
