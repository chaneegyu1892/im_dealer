  ...
  return (
    <div className="flex-1 bg-[#F8F9FC] flex flex-col relative overflow-hidden">
      <div className="flex-1 overflow-y-auto">
        {/* 썸네일 */}
        <div
          className="h-[180px] sm:h-[220px] md:h-[280px] w-full relative flex items-end p-4 md:p-8 overflow-hidden"
        ...
        >
          {vehicle.thumbnailUrl && (
            <Image
              ...
              fill
              sizes="100vw"
              unoptimized={vehicle.thumbnailUrl.startsWith("http")}
              className="absolute inset-0 w-full h-full object-cover"
            />
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
          <div className="relative z-10 text-white w-full">
            <span className="inline-block px-2.5 py-1 bg-white/20 backdrop-blur-md rounded-full text-[10px] md:text-[11px] font-medium mb-2">
              {vehicle.brand}
            </span>
            <h1 className="text-[22px] md:text-[32px] font-bold leading-tight">{vehicle.name}</h1>
            <p className="text-white/80 text-[12px] md:text-[14px] mt-1 break-keep line-clamp-2">
        ...
        {/* 기본 정보 */}
        <div className="p-4 md:p-8 max-w-[800px] mx-auto space-y-4 md:space-y-8">
      ...
      {/* 하단 버튼 */}
      <div className="bg-white border-t border-[#E8EAF0] p-4 flex justify-end shrink-0">
        <Link
          href={`/admin/vehicles/${vehicle.id}`}
          className="flex items-center gap-2 bg-[#000666] text-white px-4 md:px-6 py-2.5 md:py-3 rounded-[8px] font-medium text-[12px] md:text-[13px] hover:bg-[#1A1A6E] transition-colors shadow-md"
        >
          <Settings size={15} />
          <span className="hidden sm:inline">상세 정보 변경 (트림·이미지 편집)</span>
          <span className="sm:hidden">트림·이미지 편집</span>
