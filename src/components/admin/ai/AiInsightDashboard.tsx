...
export default function AiInsightDashboard({ data }: Props) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
      ...
      {/* 최다 추천 차량 */}
      <div className="sm:col-span-2 md:col-span-2 bg-white p-5 rounded-2xl border border-[#E8EAF2] shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-[#1A1A2E] flex items-center gap-1.5">
            <TrendingUp size={16} className="text-[#6066EE]" />
            <span className="hidden sm:inline">성능 기반 최다 추천 차량 (TOP 5)</span>
            <span className="sm:hidden">최다 추천 차량 TOP 5</span>
          </h3>
            ...
            <div key={v.vehicleId} className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-xs font-bold text-[#1A1A2E] truncate">{v.name}</span>
                  <span className="text-[10px] text-[#9BA4C0]">{v.brand}</span>
                </div>
              </div>
              <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                <div className="flex flex-col items-end">
                  <span className="text-[10px] text-[#9BA4C0]">추천</span>
