...
export async function storeKakaoRefreshToken(
  ...
  try {
    ...
    console.error("[kakao] storeKakaoRefreshToken failed:", err);
  }
}
...
export async function getKakaoAccessToken(supabaseId: string): Promise<string | null> {
  ...
  try {
    ...
    console.error("[kakao] refresh token decrypt failed:", err);
    return null;
  }
  ...
  try {
    ...
    if (typeof json.refresh_token === "string" && json.refresh_token.trim()) {
      await storeKakaoRefreshToken(supabaseId, json.refresh_token.trim());
    }

    return accessToken;
  } catch (err) {
    console.error("[kakao] getKakaoAccessToken failed:", err);
    return null;
  }
}
