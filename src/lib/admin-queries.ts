...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  // DB에 차량이 한 건도 없으면 목 데이터 통계 반환 (데모용)
  if (totalVehicles === 0) {
    const { DASHBOARD_STATS } = require("@/constants/mock-data");
    stats = {
      ...
      monthlyQuotes: DASHBOARD_STATS.monthlyQuotes,
    };
  }
...
export async function getAdminQuotes(page = 1, limit = 20): Promise<{
  ...
  // DB에 데이터가 한 건도 없으면 목 데이터를 반환 (데모용)
  if (data.length === 0 && page === 1) {
    const { MOCK_QUOTES } = require("@/constants/mock-data");
