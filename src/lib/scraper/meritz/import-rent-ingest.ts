// 메리츠 수입차 렌터카 .xlsm → CapitalCatalogTrim 엔트리 (파싱 + 우리DB 가격매칭 + 월렌트료 산출).
// 국산 렌트 ingest 와 동일 shape/우선순위(확정 매핑 > 이름 매칭). 같은 productType(장기렌트)에 브랜드만 수입으로 추가된다.
import { parseMeritzImportRentWorkbook } from "./import-rent-parse";
import { computeMonthlyImportRent } from "./import-rent-calc";
...
/** 워크북 버퍼 + 우리 차량목록 → 수입차 카탈로그 엔트리. weekOf/scrapedAt 은 라우트에서 부여. */
export function ingestMeritzImportRent(
  buf: Buffer | ArrayBuffer, ourVehicles: OurVehicle[], mappedPrices?: MappedPriceByMdelCd
): MeritzIngestResult {
  const { trims, constants } = parseMeritzImportRentWorkbook(buf);
  ...
  for (const t of trims) {
    ...
    if (price > 0) {
      for (const c of CELLS) {
        const v = computeMonthlyImportRent(t, price, c.months, c.distKm, constants);
      ...
      if (baseRates["36_10000"]) {
        const dep = computeMonthlyImportRent(t, price, 36, 10000, constants, { depositRate: 0.1 });
        if (dep && dep > 0) depositRate36_10000 = dep;
        const pre = computeMonthlyImportRent(t, price, 36, 10000, constants, { prepayRate: 0.1 });
